...
public Response assignStandsToPublishers(String standsText, String publishersText) {
    ...
    // evitar stands repetidos
    java.util.HashSet<Long> standIds = new java.util.HashSet<>();
    ...
    for (String line : standLines) {
        ...
        // NO se deben repetir stands
        if (standIds.contains(standId)) {
            return new Response(
                    "No se permiten stands repetidos en la misma compra.",
                    Status.BAD_REQUEST
            );
        }
    ...
    for (String line : publisherLines) {
        ...
        // NO se deben repetir editoriales
        if (publisherNits.contains(nit)) {
            return new Response(
                    "No se permiten editoriales repetidas en la misma compra.",
                    Status.BAD_REQUEST
            );
        }
        publisherNits.add(nit);
